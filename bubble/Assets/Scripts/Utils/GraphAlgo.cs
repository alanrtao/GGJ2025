...
namespace Utils
{
    ...
    {
        private static readonly Func<GridPoint, bool> NotBubble = GridGen.Not(GridGen.IsBubble);

        public static List<GridPoint> PickShortestPath(Dictionary<GridPoint, List<GridPoint>> paths)
        {
        ...
        public static bool TryFindPath(GridPoint start, Func<GridPoint, bool> isTarget, out Dictionary<GridPoint, List<GridPoint>> paths)
        {
            ...
            paths = hasPath.ToDictionary(p => p, p =>
            {
                ...
                for (var q = p; q != start; q = dist[q].Item1)
                {
                    path.Add(q);
                }
                path.Reverse();
...
public class PriorityQueue<TElement, TPriority>
{
    ...
    public TElement Peek()
    {
        if (_size == 0)
        {
            throw new IndexOutOfRangeException(); // InvalidOperationException(SR.InvalidOperation_EmptyQueue);
        }
    ...
    public sealed class UnorderedItemsCollection : IReadOnlyCollection<(TElement Element, TPriority Priority)>, ICollection
    {
        ...
        void ICollection.CopyTo(Array array, int index)
        {
            ...
            if (array.Rank != 1)
            {
                throw new ArgumentException("rank multidim not supported", nameof(array));
            }
            ...
            {
                throw new IndexOutOfRangeException(); // throw new ArgumentOutOfRangeException(nameof(index), index, SR.ArgumentOutOfRange_IndexMustBeLessOrEqual);
            }

            if (array.Length - index < _queue._size)
            {
                throw new IndexOutOfRangeException(); // throw new ArgumentException(SR.Argument_InvalidOffLen);
            }
        ...
        private static readonly List<(TElement Element, TPriority Priority)> emptyListToPatchIEnumeratorGetter = new();
        ...
        IEnumerator<(TElement Element, TPriority Priority)> IEnumerable<(TElement Element, TPriority Priority)>.GetEnumerator() =>
            _queue.Count == 0 ? emptyListToPatchIEnumeratorGetter.GetEnumerator() :
            GetEnumerator();
